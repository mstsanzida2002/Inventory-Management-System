...
class NumberGenerationCollisionTests(ServiceTestCase):
    """BUG-87 (docs/bugsfound.md) -- po_number/invoice_number/sku were all
    a 4-digit random suffix per calendar day (9000 values, N), no retry on
    a unique-constraint collision. Birthday-paradox math: P(no collision)
    approx e^(-n(n-1)/2N), so P(collision) crosses 50% around n=112 same-
    day rows and is >99% by n=288 (n = sqrt(2N * ln(1/(1-p)))). 300 real,
    unmocked creates per model here (all same day, same namespace) is
    comfortably past that 99% line -- if the retry-on-collision fix
    (Product.save()/PurchaseOrder.save()/SaleTransaction.save(), all via
    the shared _save_with_generated_unique_number()) weren't working, at
    least one duplicate would be virtually guaranteed to show up, not
    just possible."""

    def test_invoice_numbers_unique_under_high_volume(self):
        numbers = set()
        for _ in range(300):
            sale = SaleTransaction.objects.create(created_by=self.user)
            numbers.add(sale.invoice_number)
        self.assertEqual(len(numbers), 300, "a collision produced a duplicate invoice_number")

    def test_po_numbers_unique_under_high_volume(self):
        numbers = set()
        for _ in range(300):
            po = PurchaseOrder.objects.create(supplier=self.supplier, created_by=self.user)
            numbers.add(po.po_number)
        self.assertEqual(len(numbers), 300, "a collision produced a duplicate po_number")

    def test_sku_unique_under_high_volume_when_autogenerated(self):
        """Product's retry path is conditional (see Product.save()'s own
        comment) -- only engaged when ProductForm.clean_sku() actually
        left the sku to be generated, never for a manually-typed one.
        Drives that same flag directly rather than 300 full form
        submissions, to isolate the model-level retry behavior."""
        skus = set()
        for i in range(300):
            product = Product(
                name=f'Collision Product {i}', category=self.category, supplier=self.supplier,
                purchase_price=Decimal('1.00'), selling_price=Decimal('2.00'), sku='',
            )
            product._sku_autogenerated = True
            product.sku = product._generate_sku()
            product.save()
            skus.add(product.sku)
        self.assertEqual(len(skus), 300, "a collision produced a duplicate sku")

    def test_manually_entered_duplicate_sku_is_still_a_real_error(self):
        """The conditional retry must never silently paper over a
        genuinely duplicate, user-typed sku by quietly swapping in a
        different one the user never chose."""
        Product.objects.create(
            sku='SKU-MANUAL-DUP', name='First', category=self.category, supplier=self.supplier,
            purchase_price=Decimal('1.00'), selling_price=Decimal('2.00'),
        )
        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                Product.objects.create(
                    sku='SKU-MANUAL-DUP', name='Second', category=self.category, supplier=self.supplier,
                    purchase_price=Decimal('1.00'), selling_price=Decimal('2.00'),
                )
