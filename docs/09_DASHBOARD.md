...
No `REQ` range assigned (see note above) — with one correction, made
2026-08-24 once `RECOVERED_REQUIREMENTS.md`'s REQ 11 range surfaced this
page's own requirements directly: **REQ 11.2** (3-second load budget),
**REQ 11.9** (forecast replenishment recommendations), **REQ 11.10**
(slow-moving/dead-stock insights), and **REQ 11.15** (role-gating) all
govern this page and are now closed — see §4d below and
`docs/bugsfound.md` for the close-out. Functionally also covers
...
| AI-dependent content stays out until it's real | **RETIRED 2026-08-24 — see §4d.** Was true from Phase 8.96 through the built-vs-designed audit (`docs/bugsfound.md` BUG-76): Phase 10/11 populated `DemandForecast`/`InventoryClassification` but nothing came back to re-add this section, closing REQ 11.9/11.10 late. Both widgets now ship. |
...
### 4d. AI Insights — built 2026-08-24 (REQ 11.9/11.10), superseding everything below this line in this subsection

**The original spec here (kept below, struck through, for the record) turned out wrong on re-verification — not just stale, wrong.** `DemandForecast.objects.order_by('-created_at')[:4]` and `InventoryClassification.objects.order_by('-classified_at')[:4]` both run without error (every field name is real), but neither means what a reader would assume. `run_full_classification()` updates every active product's `classified_at` in the *same batch*, so "most recently classified" is an arbitrary, near-tied ordering, not a priority ordering — it says nothing about which products are actually dead or slow. `DemandForecast` rows accumulate by design (REQ 9.9 needs history to compare against `actual_demand`), so "most recently created" returns whatever a handful of products happened to be re-forecast last, not products that need reordering. Both queries would have rendered *something* without crashing, which is exactly why they needed re-verification against real code rather than being implemented as written — see `docs/bugsfound.md` BUG-76 for the full write-up prompting the correction.

**What actually shipped, both in `frontend/views.py`'s `DashboardView.get()`, same Supervisor+ gate as Recent Activity (Decision 5) and as the two pages they link to (`SupervisorRequiredMixin` on both `DemandForecastingView`/`SlowMovingDeadStockView`):**

- **Stock classification insights (REQ 11.10).** Real counts across all four states (`fast`/`slow`/`dead`/`insufficient_data`, via `InventoryClassification.objects.values('classification').annotate(count=Count('id'))` — the same aggregation shape `ClassificationSummaryAPIView` already uses, so this widget can never diverge from `/ai/slow-moving/`'s own counts, the exact failure mode BUG-64 was on the forecasting side). Highest-priority dead/slow products: `InventoryClassification.objects.filter(classification__in=[DEAD, SLOW]).order_by('-stagnation_index')[:DASHBOARD_PREVIEW_ROWS]`, showing `flagged_by_rule` ("No sales in 210 days") in place of the raw index whenever an override decided the row — the human-readable reason is more useful than a number. Links to `/ai/slow-moving/`. **Slot left for capital-at-risk ranking** (Step 4): the widget is a plain list with one sort key (`-stagnation_index`), so swapping or blending in a risk value is a one-line change, not a restructure.
- **Forecast replenishment recommendations (REQ 11.9).** Products where `forecasted_demand > current_stock`, **weekly forecasts only** — matching `run_full_forecast()`'s own `replenish_alerts` condition exactly, not a broader definition invented for this widget (a monthly forecast exceeding stock is a much weaker signal for otherwise-healthy, more-than-monthly-restocked inventory). Ordered by deficit (`forecasted_demand - current_stock`) descending — the larger the shortfall, the more urgent. Sourced from `frontend.forecasting.latest_forecast_batch()` (extracted from `DemandForecastingView`'s own dedup-by-latest-run logic specifically so this widget shares one definition of "current forecast," not a second one), **deliberately not `ForecastSummaryAPIView`**, which has a known, still-open aggregation defect (BUG-64: no dedup by run, so repeated "Run forecast now" clicks skew its counts). Shows `recommended_reorder_qty` and `confidence_score`. Links to `/ai/forecasting/`.
- **Performance:** dashboard load timed before/after on the live dev server, 5 requests each, warm: ~0.20-0.30s before, ~0.30-0.39s after — a real but small increase, nowhere near the 3-second budget (REQ 11.2). Not cached or deferred; the extra cost is 4 small queries against tables with under 200 rows each on this dataset.
- **Empty states:** both widgets render cleanly with zero classifications, zero forecasts, and both empty at once (a genuinely fresh install) — "No slow-moving or dead stock flagged right now." / "No forecasts have been run yet." (vs. "No products currently need replenishment" once a run has happened but nothing needs reordering) — never a crash, never a fabricated number.
...
8. **Approved as-is, updated 2026-08-24.** Role visibility resolved as **"Any role, same content"** (per `API_CONTRACTS.md`'s concrete endpoint table), not `INDEX.md`'s vaguer "role-specific views" — a doc inconsistency, surfaced rather than silently picked either way. This is the page-wide default; **Recent Activity (Decision 5) and the two AI Insights widgets (§4d) are the deliberate exceptions** — admin/supervisor-only, same boundary as the pages they link to, everything else on the page is identical for every role. AI Insights: was dropped pending Phase 10/11 (original recommendation, correct at the time); now built, see §4d for what shipped and why the original re-add query shape needed correcting first.
