...
## Phase — Batch of Four Low-Risk Cleanups (2026-08-25)

**1. Invoice/PO/SKU number collision (BUG-87) — reclassified as a real
defect, fixed.** `PurchaseOrder.po_number`/`SaleTransaction.
invoice_number` both already carried `unique=True`, so the failure mode
was never a silent duplicate — it was an uncaught `IntegrityError`
crashing a real user's request mid-sale on a genuine collision.
Birthday-paradox math on the 4-digit/9000-value daily namespace: 50%
collision odds at n≈112 same-day rows, >99% by n≈288 — not a remote
edge case. New shared `_save_with_generated_unique_number()`
(`frontend/models.py`) retries the save under a fresh number on
`IntegrityError`, each attempt in its own `transaction.atomic()`
(nests as a SAVEPOINT when the caller already has an outer transaction
open, a real transaction otherwise — either way a failed attempt never
leaves a partial row and never collides with a concurrent request's own
in-flight number; the DB's unique index is the actual safety guarantee,
this only stops it from surfacing as a crash). `PurchaseOrder.save()`/
`SaleTransaction.save()` use it unconditionally. A third instance
turned up while checking "any other model": `ProductForm._generate_sku()`
had the identical pattern (plus its own separate, pre-existing
`timezone.now()` UTC-date bug, now also fixed to match Phase 8.99's
`timezone.localdate()` precedent). Product's fix is conditional —
gated on a `_sku_autogenerated` flag `clean_sku()` sets only when the
user left SKU blank — because a manually-typed duplicate SKU must
still surface as a real validation error, never be silently swapped
for a value the user didn't choose. Tests: `NumberGenerationCollisionTests`
(4), 300 real unmocked creates per model. Live-verified: 20 real sales
created via actual HTTP POSTs in quick succession, all 20 invoice
numbers unique (confirmed both from the rendered page and directly
against the DB).

**Worth recording on its own, not just as a narrative detail above: the
task that reported this bug named only invoices — the same collision
pattern turned out to exist in three unrelated places** (`PurchaseOrder.
po_number`, `SaleTransaction.invoice_number`, and `ProductForm`'s
auto-generated `Product.sku`), found only by explicitly checking "does
any other model use the same generation pattern" rather than trusting
the original scope. The lesson isn't the specific bug, it's the shape:
**a "4-digit random suffix, no retry" ID-generation idiom got copy-
pasted three times across this codebase before anyone noticed the
birthday-paradox risk in any of them.** If a future session finds a
fourth instance of this exact pattern (grep `random.randint(1000,
9999)` first), or a similarly-shaped copy-pasted idiom elsewhere, that
"check every other model, don't assume the report's own scope is
complete" instinct is the one to reach for again — it's what surfaced
this the first time.

**2. Reject/Cancel/Deactivate icons (REQ 18.12) — verified, no code
change.** Confirmed via direct grep of every template: Reject
(`icon-circle-slash`) is genuinely distinct from Cancel/Deactivate
(both `icon-x`) — the originally-flagged confusion is fixed. Residual
overlap is real but lower-severity: Cancel (Purchases/Sales) and
Deactivate (Products/Categories/Suppliers/Users/Approval Policies)
share `icon-x`, but never appear on the same page together, so a user
is never asked to visually distinguish them side by side. No dedicated
"deactivate" icon exists in `includes/icons.html`'s sprite — adding one
is a real (if small) design decision, not a same-session cleanup, so
left as a reported finding rather than acted on.

**3. BUG-63 (partial final resample bin) — already fully disclosed,
verified not just claimed.** `docs/DEMAND_FORECASTING.md` Design Note
#12 already states the mechanism plainly (in-progress week/month →
partial bin → biased `lag_1` for the first forecast step only). One
adjacent staleness fixed while here: Design Note #11 still said BUG-64
was "not fixed" — it was, in the immediately preceding pagination-pass
session (`ForecastSummaryAPIView` now calls `latest_forecast_batch()`)
— corrected to say so and point at `docs/bugsfound.md`.

**4. Record unlock system — already disclosed, given a second,
more discoverable home.** Confirmed (again) fully phantom — no model,
field, migration, or code path named anything like it, re-run with the
same exhaustive search an earlier phase (12.1) already used. The only
existing disclosure lived in `docs/project_memory.md` (a chronological
session log); no requirements/spec doc in `docs/*.md` ever named this
feature, because its source material (`requirement_analysis_doc_2.docx`)
doesn't exist in this repo. Logged as BUG-88 in `docs/bugsfound.md` too
— the structured log a reader would actually check first — rather than
building or deleting anything; the re-resolution contract from Phase
12.1 (increase → invalidate & return to pending; unchanged → keep, log;
decrease → keep, never auto-post) is preserved there for whoever
eventually builds this for real.

Full suite: 440 -> 444 (net +4, `NumberGenerationCollisionTests`). No
templates touched this pass. Not committed — reported back for review
per instruction.

---

## Phase — Two AI-Page Defects: Stale Forecast Selection, Methodology-as-Insight (2026-08-26)

**Defect 1 (BUG-89) — architectural note for future sessions, so this
isn't re-solved by breaking the wrong function.** `frontend.forecasting.
latest_forecast_batch()` returns the full accumulated pool of every
period ever forecast across every run, deduped only per `(product,
forecast_period, period_start)` — this is correct and stays exactly as
it is, because REQ 9.9 needs that full history retained to compare
against `actual_demand` once backfilled. That function answers "what
has ever been forecast," not "what should a user see right now" — those
are different questions, and conflating them is exactly what caused
BUG-89 (the trend chart and table both picked the chronologically
*oldest* period once forecasts had accumulated across enough runs).
`frontend.forecasting.current_forecast_window(forecast_period,
horizon=4)` is the correct entry point for "what's current": it layers
a `period_start >= today` filter (plus an optional distinct-period cap)
on top of `latest_forecast_batch()`'s pool, and both the chart and the
table's per-product selection go through it now. **Any future work on
"what forecast should this page show" belongs in
`current_forecast_window()`, not in a third reimplementation, and
`latest_forecast_batch()`'s own contract should not be narrowed to fix
this again.**

**Defect 2 (BUG-90).** Slow-Moving's "AI Insight" block was static
methodology text since the page was first built (confirmed via git
history across three prior versions of the paragraph, oldest to
newest) — the panel removal earlier in this session didn't cause it,
it just made the gap conspicuous by leaving this as the only "AI"
content on the page. Replaced with real per-run findings (dominant
finding in plain language, most-urgent product's `flagged_by_rule`,
stock value at risk, last-classified timestamp), reusing exactly what
the view already computed rather than adding new classifier logic.
`capital_at_risk()` moved from `frontend/reports.py` (`_capital_at_risk`,
private) to `frontend/classification.py` (public) so the classification
report's export and this page's insight share one definition instead
of a near-duplicate. `slow_index_threshold`/`dead_index_threshold`/
`min_observation_days` context keys, only read by the removed
paragraph, dropped from `SlowMovingDeadStockView`'s context — same
disclosure pattern as `target_days_of_cover`/`extreme_coverage_days`
in the pagination pass, so these don't get rediscovered as a mystery
either.

**Also traced, not a bug**: the Monthly chart's September bar (636.3)
looked like a ~65x spike against July/August (9.7/9.2) — real numbers,
correctly summed across 29 products for September vs. 1 stale product
each for July/August. Once `current_forecast_window()` filters both
consumers to current-or-future periods, July and August (both before
today) disappear from the pool entirely, and the misleading comparison
no longer exists — nothing needed fixing in the aggregation itself.

Full suite: 444 -> 457 (net +13: `ForecastCurrentWindowTests` (7),
`SlowMovingAIInsightTests` (5), one extended assertion in
`RemovedExplainerPanelTests`). Live-verified on the dev server for
both pages — not just tests, since both defects were the "200 OK but
wrong" class assertions alone can't catch. Not committed — reported
back for review per instruction.

---

## Phase — Price Auto-Fill, Product Form Simplification (2026-09-05)

**Auto-fill (Purchase/Sale forms).** Mirrors the existing `data-tax-rate`
pattern exactly, no new mechanism: `data-purchase-price`/`data-selling-
price` added to the server-rendered product `<option>` tags
(`purchases.html`/`sales.html`), read by `line-items.js`'s existing
product-select `change` handler via a new `priceAttr` config key. A
product with no price (can't happen today — 0 of 45 real rows are null
on either price field, but the model doesn't forbid it structurally)
renders the attribute empty (`{{ product.purchase_price|default_if_none:
'' }}`), and the JS leaves the input empty rather than inserting 0.

**The storage contract did not change, deliberately.** `PurchaseOrderItem.
unit_price`/`SaleItem.unit_price` were already copied at creation time,
never read live from `Product` — confirmed by discovery *before* writing
any code, and re-confirmed by a new explicit historical-integrity test
per model (`PriceAutoFillTests`) that creates a transaction, changes the
product's price afterward, and asserts the stored `unit_price`/
`line_total` are unchanged. Auto-fill only changes what the form shows
*before* submission; the server still trusts whatever `unit_price` the
client actually submits, exactly as before. Tax is unchanged and stays
the one exception: always re-derived server-side from `Product.tax_rate`,
never the client's value.

**reorder_level: kept on the form, reversing the original instruction on
the discovery's own evidence** — six real dependents (`InventoryRecord.
update_status()`'s LOW_STOCK threshold, the Products page's own status
filter/KPI counts/CSV export, the Inventory/Low-Stock reports, seed data)
plus 8 distinct values genuinely in use across 45 seeded products meant
removing it would have been a real capability regression (every new
product collapsing to one system-wide default), not a simplification.
**If a future session is asked to "simplify the product form" again,
reorder_level is not a free target** — re-read this note and the
discovery in `docs/03_PRODUCTS.md`'s own "Implementation Notes" section
before touching it again.

**description/image: removed from ProductForm and both Add/Edit
templates only — the model columns are deliberately still there,
unused.** Confirmed before removing anything: no product detail page
exists to display either, no report/PDF reads them, no serializer
exposes Product at all. Dropping the model columns was explicitly out
of scope (irreversible, buys nothing a form-level removal doesn't
already buy) — **if a future session finds these two columns present
but unreachable through any form and wonders why, this is why: it's
deliberate, not an oversight, and they're sitting there ready if a
product detail page is ever built.** `views.py`'s edit-modal pre-fill
JSON (`ProductListCreateView.get()`) no longer sends either field,
since the edit form can no longer accept them back.

**tax_rate made required** (BUG-91, docs/bugsfound.md) — was silently
defaulting a blank submission to 0%, the one price-shaped field on this
form that worked differently from `purchase_price`/`selling_price`. A
deliberate 0 is still valid and accepted (11 of 45 real products
already carry it); what changed is that it must be typed now, not
inferred from an empty box.

Full suite: 457 -> 467 (net +10: `PriceAutoFillTests` (7),
`ProductPriceAutoFillTemplateTests` (2), `ProductTaxRateTests` net +1).
Live-verified via a real headless-browser session (Playwright, already
installed in this project's `.venv`, no new dependency added) rather
than HTTP-only simulation, since the auto-fill behavior itself is
client-side JS that a plain request can't exercise: selected a real
product on both the Purchase and Sale forms, confirmed the price input
auto-filled from the correct data attribute, manually overrode it,
submitted, and confirmed via direct DB query that the *overridden*
value was stored, not the product's current price. Also created a
product through the simplified Add Product form (no description/image
fields present) and confirmed it appeared correctly in the product
list, the "Out of stock" filter, the KPI strip (total 45 -> 46), the
CSV export, and a generated inventory PDF. Test data cleaned up after
(the created product deleted; the draft PO/sale left in place as
