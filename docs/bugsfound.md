...
| BUG-92 | **Loading the Reports page logged a `REPORT_GENERATED` audit entry unconditionally, on every GET, whether or not anyone actually generated or exported anything — false data, not merely wasted work.** `ReportsView.get()` built the Sales and Low Stock inline preview panels' aggregate data (a status-breakdown table + chart for Sales, a per-product table for Low Stock) on every page load and logged one `REPORT_GENERATED` entry per panel each time, regardless of whether the visitor ever clicked an export. A supervisor merely opening `/reports/` to look around would leave an audit trail claiming they "generated" two reports they never asked for — the query work behind those two panels was pure waste as well, once nothing else on the page read it. Checked for the same pattern elsewhere: `REPORT_GENERATED` was fired from exactly these two call sites and nowhere else in the codebase (confirmed by grep across `frontend/*.py`); `INVENTORY_VIEWED` (`frontend/views.py`, `InventoryListView`) is structurally similar — an audit entry fired on every GET — but is **not** the same defect, since that page's entire purpose is genuinely showing live inventory data on every load; the log entry there is truthful | `ReportsView.get()` (`frontend/views.py`), `frontend/templates/reports/reports.html`'s now-removed `salesReportPanel`/`lowStockReportPanel` blocks | ✅ **Fixed**, as part of the Reports-page cleanup (docs/project_memory.md). Both inline preview panels removed; every one of the 9 reports is now a uniform PDF/CSV download card, with Sales and Low Stock the only two carrying filter inputs (date range + category for Sales, category for Low Stock), read at export-click time and appended to the download URL. `ReportsView.get()` now only renders the card grid — no aggregate queries, no audit call. `REPORT_GENERATED` (`frontend/audit.py`) is now permanently unreachable — nothing calls it anymore; kept as a constant rather than deleted, matching this codebase's existing precedent for retired-but-referenced audit actions. `REPORT_EXPORTED_PDF`/`REPORT_EXPORTED_CSV` are untouched and still fire correctly, only on a real export. Tests: `ReportsPageCardUniformityTests` (12, including `test_loading_the_reports_page_logs_no_report_generated_entry` and `test_exports_still_log_the_correct_export_action`). Live-verified on the dev server (Playwright, admin session): loaded `/reports/` and confirmed zero new `REPORT_GENERATED` rows; downloaded Sales CSV/PDF and Low Stock PDF (unfiltered, then with a date range and a category filter applied) and confirmed exactly 5 new `REPORT_EXPORTED_PDF`/`REPORT_EXPORTED_CSV` rows, one per real download, correct `report` value in each `details` payload; confirmed the date/category filters genuinely narrow the exported file (1423 rows unfiltered → 1 row for a real 5-day window, 0 rows for an out-of-range window; 3 low-stock products unfiltered → 1 for a single category) rather than being cosmetic |
| BUG-93 | **`frontend/mixins.py`'s own module docstring falsely claimed its RBAC mixins were "Not yet applied to any real class-based view," proven only against a throwaway test CBV — a documentation defect with real exposure, not a style issue.** By the time this was found, `AdminRequiredMixin`/`SupervisorRequiredMixin`/`AnyStaffMixin` (all subclasses of `RoleRequiredMixin`, defined in this same file) were the actual authorization gate on 83 separate call sites across `frontend/views.py` — every role-restricted page in the app. A reader trusting the docstring (a reviewer, an auditor, a future maintainer deciding what to test) would conclude this RBAC layer was unused scaffolding and could reasonably skip auditing it, when it is in fact the mechanism enforcing every Admin/Supervisor/Staff boundary in the project. `frontend/decorators.py` carries a near-identical docstring claim ("not yet applied to any real view") — checked separately and confirmed still *true*: only `frontend/tests.py` imports `require_role`/`admin_required`/`supervisor_required`/`staff_required`, so that file was not misdiagnosed as the same bug just because it reads similarly. Found during the comment-hygiene pass's Phase A discovery, confirmed by grep (`RoleRequiredMixin\|AdminRequiredMixin\|SupervisorRequiredMixin\|AnyStaffMixin` → 83 hits in `views.py`) before any edit was made | `frontend/mixins.py`'s module docstring | ✅ **Fixed.** Docstring rewritten to state plainly that this is the real, actively-used RBAC gate across the app, with this BUG-93 cross-reference so the correction itself doesn't silently rot the same way. `frontend/decorators.py`'s own docstring left with its "not applied" claim intact (still accurate) but clarified to name `mixins.py` as the mechanism actually in use, so a future reader isn't left wondering why two near-identical RBAC layers exist. No code changed — this was a comment-only fix, verified via `scripts/comment_guard.py`'s AST-equivalence check. *Session 2 note:* the comment policy tightened to ban `BUG-xx` tags in code outright; the retrofit pass removed the `BUG-93` cross-reference this fix originally left in `mixins.py`'s docstring — the cross-reference now lives only here |
| BUG-94 | **`audit.log_action()`'s own docstring claimed its `request` parameter "is always None for now" because "no views exist yet to pass one" — false by a wide margin at the time this was found.** `grep -c "request=request" frontend/views.py` returns 39 — every one of those call sites passes a real Django request through to `log_action()`, which then extracts a real client IP via `X-Forwarded-For`/`REMOTE_ADDR`. The claim was true only in the phase this module was first written, long before `views.py` existed, and was never updated once views started passing a real request on every call. The code itself was never wrong — `if request:` correctly handles both a real request and `None` — only the docstring lied about which case was live | `frontend/audit.py`'s `log_action()` docstring | ✅ **Fixed.** The false claim was deleted outright rather than rewritten, per this session's comment policy — a stale claim about a parameter's runtime value doesn't belong in code once it no longer holds, and the surrounding code already speaks for itself. No code changed — comment-only fix, verified via `scripts/comment_guard.py` |
| BUG-95 | **`notifications.py`'s own module docstring claimed `notify_user()`/`notify_supervisors()` are "the ONLY code path allowed to create Notification rows" — false, `notify_admins()` (defined in the same file) also calls `Notification.objects.create()` directly.** The claim was accurate when written, before `notify_admins()` existed, and was never updated once it was added as a third path. Verified via `grep -rn "Notification.objects.create" --include="*.py" .` (excluding migrations): exactly 3 application-code sites, all three of `notify_user()`/`notify_supervisors()`/`notify_admins()` in `frontend/notifications.py` — confirming the *true* rule (all three `notify_*` helpers, not two) is real and worth stating, not a claim to drop entirely. (`frontend/tests.py` also creates `Notification` rows directly 4 times, for fixture setup — expected test practice, not a violation of the application-code rule) | `frontend/notifications.py`'s module docstring | ✅ **Fixed.** Corrected to name all three `notify_*` helpers rather than two, condensed to the new one-line comment form. No code changed — comment-only fix, verified via `scripts/comment_guard.py` |
| BUG-96 | **`ApprovalPolicy.notes` — a free-text field admins fill in when authoring an approval rule — carries at least one seeded policy row whose text embeds a phase reference (`"...so this can't be salami-sliced (Phase 12.1 §4)."`), and that text is a live exposure in the running application, not just a database curiosity.** No template directly renders `{{ policy.notes }}` (checked: zero direct interpolations anywhere in `frontend/templates/`), but `frontend/views.py:2837` serialises `"notes": policy.notes` into a per-row `edit_json` blob embedded in the Settings page's HTML (the same server-computed, client-read-via-data-attribute pattern this project already uses for Product's own edit modal), and `frontend/templates/settings/_approval_policy_fields.html:90`'s `<textarea id="{prefix}-notes">` is populated from that JSON when an admin clicks Edit on the row. The phase-reference text is therefore present in the page's raw HTML source on every load of the Settings page's Approval Policy section, and visibly populated into the textarea on Edit — exactly the kind of thing an examiner clicking through the Settings page would see. Found during the comment-hygiene pass's session-3 discovery, while confirming the seeded `notes` string flagged (but correctly left alone, as data) in session 2 doesn't also surface in the UI | `frontend/views.py`'s `edit_json` serialization (line 2837), `frontend/templates/settings/_approval_policy_fields.html` (line 90), the seed data itself (`frontend/approvals.py`'s `DEFAULT_APPROVAL_POLICIES`) | ⚠️ **Not fixed.** Both the data fix (rewriting the seeded policy's `notes` text) and the code fix (`views.py`) fall outside this session's scope (`frontend/approvals.py`'s comments were already retrofitted in session 2; `views.py` is scheduled for session 4). Logged here so it isn't lost, and so a future session — or whoever authors real production approval policies — knows this field is admin-facing UI copy, not an internal changelog |
| BUG-97 | **`frontend/admin.py` carried three separate documentation defects, all in `ModelAdmin` docstrings for `InventoryMovementAdmin`, `AuditLogAdmin`, and `SystemSettingsAdmin`.** (1) All three docstrings ended with "(see admin_notes below)" — no `admin_notes` section, variable, or comment exists anywhere in this file or the codebase (confirmed by grep); a dangling forward-reference to content that was never written, the same class of defect as BUG-93's false claim in `mixins.py`. (2) Two of the three also asserted the *opposite* of what the model code does: `InventoryMovementAdmin`'s docstring claimed the model "doesn't enforce" its immutability invariant "unlike AuditLog" — false; `InventoryMovement.save()`/`.delete()` (`frontend/models.py:417-423`) raise `PermissionError` exactly like `AuditLog` does. `SystemSettingsAdmin`'s docstring claimed "the model itself doesn't enforce single-row-only" — false; `SystemSettings.save()` forces `self.pk = 1` unconditionally, which is real enforcement, not just a `get_settings()` convention. Found during the comment-hygiene pass's session-4 discovery, while removing the dangling `admin_notes` references; the two false claims were caught by reading the actual model code before deleting rather than deleting on the assumption the docstrings were merely stale-but-harmless | `frontend/admin.py`'s `InventoryMovementAdmin`, `AuditLogAdmin`, `SystemSettingsAdmin` docstrings | ✅ **Fixed.** All three docstrings removed outright rather than rewritten (the dangling reference and the two false claims are both defects to delete, not preserve in corrected form, per this session's comment policy). The real, verified rationale for each `has_change_permission`/`has_delete_permission`/`has_add_permission` override was written back as a one-line comment directly above the method it explains, stating the true mechanism (`PermissionError` on `InventoryMovement`/`AuditLog`; `pk=1` forcing on `SystemSettings`). No code changed — comment-only fix, verified via `scripts/comment_guard.py` |
| BUG-98 | **`async-run-button.js`'s own header comment claimed Demand Forecasting used a "simulated" (client-side-only, no real Celery task) code branch — false at the time this was found; neither of the button's two current callers (`forecasting.html`'s "Run forecast now", `slow_moving.html`'s "Run classification now") uses a simulated branch, both hit a real synchronous server action.** The comment described a code path that no longer exists on either page using this module; a reader would wrongly conclude one of these buttons is a no-op mock and skip auditing what it actually does (same class of defect as BUG-93/96/97: a stale claim actively misleading rather than merely out of date) | `frontend/static/js/async-run-button.js`'s header comment | ✅ **Fixed.** Found during this session's (5/5) comment-hygiene Phase A discovery of `frontend/static/**/*.js`. False claim deleted, replaced with `// Assumption: config.action is a real synchronous call -- no Celery task queue.` — the true, current fact, stated once at the top of the file rather than tied to either specific caller. No code changed — comment-only fix, verified via a custom strict-diff check (comment_guard.py's generic HTML/JS/CSS heuristic has known false-positive gaps for this shape of edit — see this session's own PHASE C note in `docs/project_memory.md`) |
| BUG-99 | **`components.css`'s `.empty-state` block carried a comment directly contradicted by the code two lines below it.** The comment claimed "no conflicting `display` rule exists for a plain div, so no override needed"; immediately following, `.empty-state { display: flex; ... }` sets exactly such a rule, and `.empty-state[hidden] { display: none; }` immediately after *is* the override the comment says isn't needed — the second comment (correctly) explains why the override IS needed, directly beside the first comment claiming it isn't, the two comments unable to both be true | `frontend/static/css/components.css`'s `.empty-state` / `.empty-state[hidden]` rules | ✅ **Fixed.** Found during this session's (5/5) CSS comment-hygiene pass. False comment deleted; the correct one (`display:flex` beats the UA `[hidden]` rule; restated per instance since CSS has no cross-file dedup mechanism) kept, compressed to one line. No code changed — comment-only fix |
| BUG-100 | **Three live documentation-drift findings surfaced during this session's (5/5) template pass, flagged rather than fixed — none are comment-only fixes, each needs a product/scope decision.** (1) The topbar's notification dropdown (`includes/topbar_actions.html`) renders permanently static, hardcoded notification entries never wired to `notifications.js` or any real `Notification` query — a visible mock masquerading as live data on every page that includes the topbar. (2) `includes/sidebar.html`'s nav links are still hardcoded literal paths (`/dashboard/`, `/products/`, etc.) rather than `{% url %}` tags; a deleted comment on this file used to note this was temporary until the named routes existed — the routes have existed since at least session 4, but the hrefs were never swapped over, so the temporary state silently became permanent. (3) `inventory/movement_history.html` carried its own comment claiming `table-filter.js` "is no longer loaded on this page (still used by Forecasting/Slow-Moving)" — false at the time this session found it; grep confirms `table-filter.js` is only loaded by `adjustment-form.js`/`supplier-form.js`/`user-form.js`'s pages now, not Forecasting or Slow-Moving (that comment, being pure stale narrative plus a false claim, was deleted rather than corrected in place, consistent with this session's policy of deleting narrative rather than preserving it) | (1)+(2) N/A — implementation gap, not sourced from a doc; (3) N/A — implementation bug, a stale in-code claim | ✅ **(1) is fixed** (bug-fix session, separate from the comment-hygiene pass) — new `NotificationRecentView` (`GET /notifications/recent/`), reusing `_NOTIF_ICON`/`DASHBOARD_PREVIEW_ROWS` and the existing mark-read/unread-count endpoints; `notifications.js`'s poll now renders the dropdown's list from that response as well as driving the dot/sidebar badge; `topbar_actions.html`'s three hardcoded rows replaced with a JS-populated container. 6 new tests (`NotificationViewTests`, including a dedicated cross-user-leak test asserting on notification id, not title), full suite 489/489. 🚩 **(2) still reported, not fixed** — sidebar hrefs remain hardcoded literal paths, explicitly out of this session's scope. **(3) is fixed** as a side effect of deleting the block that contained the false claim, verified via `scripts/comment_guard.py`'s AST-diff-adjacent custom check |
| BUG-101 | **`Product.purchase_price`/`selling_price` and `PurchaseOrderItem`/`SaleItem.unit_price` accepted zero — `ProductForm.clean_purchase_price()`/`clean_selling_price()` and `parse_line_items()` only ever rejected `< 0`, and the model fields themselves carried no validators at all, so `0` passed every layer. Django admin's `ProductAdmin` has no `form =` override, so it ran no validation whatsoever — a laxer door than the real form, negative values included. No downstream code divides by a price (`capital_at_risk()`, `calculate_line_total()`, `InventoryRecord.total_value` all multiply), so this never crashed anything — it silently produced zero-valued money figures instead. Two same-day dev rows already existed with a zero field (`Flagylq` purchase_price=0.00, `Medical2` selling_price=0.00), both manual Add-Product-form test artifacts, neither from `seed_dev_data.py`** | N/A — implementation bug (no validator, on either the model fields or the two clean_*() methods, was ever written) | ✅ **Fixed.** `MinValueValidator(Decimal('0.01'))` added to all four fields (`frontend/models.py`), the real gate every entry point — including admin — now inherits (migration `0015`, a state-only `AlterField`, no `ALTER TABLE` on Postgres since validators aren't DDL). `ProductForm`/`parse_line_items()` (`frontend/forms.py`) now give a human message ahead of the model's generic one ("Purchase price must be more than 0.", "Selling price must be more than 0.", "Line N: unit price must be more than 0."). No `selling_price >= purchase_price` rule added — deliberately out of scope, selling below cost is a legitimate business call. New `PriceValidationModelTests` + `ProductAdminPriceValidationTests`, plus 0/negative/0.01 cases added to `ProductCreateViewTests`/`PurchaseWorkflowViewTests`/`SaleWorkflowViewTests`; full suite 512/512 (489 baseline + 23 new, zero existing tests needed changing). **The two existing zero-price rows are now resolved, asymmetrically, by design.** `Flagylq` (purchase_price=0, no transaction history) was deleted via the same two-step `ProductDeleteView` already uses (`InventoryRecord` first, then `Product`) — gone. **`Medical2` (sku `PRD-20260914-6722`, selling_price=0) survives, deactivated (`is_active=False`), with its zero `selling_price`, its `InventoryRecord`, its received PO line (`PO-20260914-3988`), and its `InventoryMovement` all left untouched.** It was *not* deleted: doing so would require force-deleting that PO line and movement row, destroying real transaction history to fix a display price — exactly what `ProductDeleteView` itself already refuses to do for any product with history, and no better when done by hand outside the view. It was *not* corrected either: inventing a plausible `selling_price` for a row backing a real received PO would be fabricating a number, not fixing one. Deactivation only removes it from `Product.objects.filter(is_active=True)` call sites — confirmed empirically: gone from the classifier's input set (`run_full_classification`) and from the Purchase/Sale line-item product dropdowns (can no longer be bought or sold at this price going forward). **It is not hidden — deactivation is not full concealment.** `ProductListCreateView`'s product list and `ProductExportView`'s CSV (`frontend/views.py`, via `filters.filter_products()`, which never filters on `is_active`) both still list it, `${{ product.selling_price }}` rendering literally as `$0.00` in the "Unit price" column — and the row carries no "Inactive" badge or styling, only a reactivate-vs-deactivate icon visible to Admin/Supervisor roles only (a Staff viewer sees no distinguishing mark at all). `InventoryRecord`-based views (Inventory page, inventory/low-stock/out-of-stock reports) also still list it, but none of those render `selling_price`, so no zero-price figure surfaces there. An examiner opening the Products page will still see this row and its `$0.00` — that is the accepted, deliberate residue of choosing deactivation over destroying transaction history. **Update:** the "no distinguishing mark at all" gap this paragraph describes is fixed by BUG-102 — the row's `$0.00` is still visible, but it now carries an "Inactive" badge, to every role, explaining why |
| BUG-102 | **Products had no visible inactive-state marker at all — Categories and Suppliers both deliberately list inactive rows (soft-delete, so Admin/Supervisor can find and reactivate them) *and* both show a visible inactive marker; Products listed inactive rows the same way but showed nothing, so Staff (who has no deactivate/reactivate button to infer state from) could not tell a deactivated product from an active one.** Directly relevant to BUG-101's surviving `Medical2` row: deactivating it does not conceal its `$0.00` `selling_price`, and until this fix nothing on the page explained why a $0.00 row was sitting there active-looking | N/A — implementation gap, not sourced from a doc (Categories/Suppliers got this treatment, Products simply never did) | ✅ **Fixed.** Scope deliberately narrower than first proposed: Products already owns the word "Status" for its stock badge (In stock/Low stock/Out of stock, `ProductListCreateView`-built) — copying Suppliers' own dedicated "Status" column (Active/Inactive) would have put two different facts under one header. Matched Categories' fold-in approach instead: `frontend/templates/products/products.html`'s SKU cell-sub now appends `<span class="badge badge-danger">Inactive</span>` when `not product.is_active` — inactive-only (nothing renders for the 45/46 active rows; badging every row would be noise, not signal) and unconditional (no role gate — Staff seeing it is the entire point). Reuses the existing `badge`/`badge-danger` CSS classes, no new CSS. One template edit only: no view change, no new `counts.inactive` tile, no client-side status filter (Suppliers has both; explicitly skipped as not worth the cost for what's currently a 1-row case — can be added later if the count becomes operationally interesting). 2 new tests in `ProductUpdateDeactivateViewTests`: badge renders exactly once and only for the deactivated product, visible to a Staff login; an all-active list renders zero badges. Full suite 514/514 (512 baseline + 2 new) |
