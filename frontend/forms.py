...
class ProductForm(forms.ModelForm):
    """description/image removed from Meta.fields (form simplification
    pass, docs/bugsfound.md) — both remain real Product model columns,
    deliberately not dropped (reversible, unlike a column drop), just
    unreachable through this form now. Discovery confirmed neither was
    ever read anywhere in the UI (no product detail page exists, no
    serializer exposes Product, no report/PDF reads either) — a clean
    simplification with no functional loss. See project_memory.md."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        # tax_rate is deliberately left at its natural ModelForm-required
        # default (form simplification pass) — a user must state 0
        # explicitly rather than have it inferred from a blank box. See
        # clean_tax_rate() below; this field no longer has a `.required =
        # False` override the way unit/reorder_level above still do.
    ...
    def clean_sku(self):
        ...
        if sku:
            return sku
        # BUG-87 (docs/bugsfound.md) — flags this instance so Product.save()
        # knows it's the one retrying under a freshly generated sku on a
        # collision, not a manually-typed one (see that method's own
        # comment for why the two cases can't share the same retry path).
    ...
    def clean_tax_rate(self):
        # Form simplification pass — tax_rate is now a required field
        # (see __init__), so Django's own required-field check already
        # rejects a blank submission before this method ever runs; the
        # old "blank -> 0" fallback is gone, a user must type 0
        # deliberately. value is never None here as a result.
